	...
	while(i<teamArr.length){
		...
		for (var j = 0; j <= 1; j++) {
			...
			if (typeof teamArr[i+j] != "undefined"){
				...
				btn.innerText=teamArr[i+j].name.toUpperCase()+"\n"+"Rank:"+teamArr[i+j].rank+"\n"+"Jersey color:"+teamArr[i+j].color+"\n"+"Captian:"+teamArr[i+j].captain+"\n"+"Cups Won:"+teamArr[i+j].worldCups;
...
function init(){
	container.innerHTML="<h3>CLICK ON TEAM FOR MORE INFO</h3>";
